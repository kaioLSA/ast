...
export function Topbar() {
  ...
  return (
    <header className="relative z-10 h-14 flex items-center justify-between px-6 border-b border-white/8 bg-[#111118]/80 backdrop-blur-md shrink-0">
      {/* Left: page title */}
      ...
      <div className="flex items-center gap-2.5 flex-1 max-w-sm mx-6">
        <button
          ...
          className="flex items-center gap-2.5 w-full h-9 rounded-xl bg-white/5 border border-white/8 px-3 hover:border-white/15 hover:bg-white/8 transition-colors text-left"
        >
          <Search className="w-3.5 h-3.5 text-slate-500 shrink-0" />
          <span className="flex-1 text-sm text-slate-600">Pesquisar...</span>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={panelRef}>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 rounded-full bg-blue-500 border-2 border-[#111118] text-white text-[9px] font-bold flex items-center justify-center px-0.5 leading-none">
          ...
          {/* Panel */}
          {open && (
            <div className="absolute right-0 top-full mt-2 w-80 rounded-2xl border border-white/10 bg-[#16161e] shadow-[0_20px_60px_rgba(0,0,0,0.6)] z-50 overflow-hidden">
              {/* Header */}
        ...
        <div className="flex items-center gap-2 pl-2 ml-1 border-l border-white/8">
          <div className="w-7 h-7 rounded-full overflow-hidden bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-xs font-bold text-white shadow-[0_0_10px_rgba(59,130,246,0.3)] shrink-0">
            {user?.avatar
